...
func (s *Synth) ActivateKeyTuning(id TuningId, name string, tuning [128]float64, apply bool) int {
	n := C.CString(name)
	defer C.free(unsafe.Pointer(n))
	return int(C.fluid_synth_activate_key_tuning(s.ptr, C.int(id.Bank), C.int(id.Program), n, (*C.double)(&tuning[0]), cbool(apply)))
}
